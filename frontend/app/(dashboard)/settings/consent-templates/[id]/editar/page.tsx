...
export default function EditConsentTemplatePage() {
  ...
  const { data: template, isLoading: templateLoading } = useConsentTemplate(templateId);
  const { mutate: updateTemplate, isPending } = useUpdateConsentTemplate();
  ...
  // Pre-populate form when template loads
  React.useEffect(() => {
    ...
    <div className="max-w-4xl space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-[hsl(var(--muted-foreground))] mt-1">
          Modifica la plantilla de consentimiento informado.
        </p>
      </div>
