"""Unit tests for the NequiServiceMock class.

Tests cover:
  - generate_qr_payment: returns a valid NequiQRResponse
...
requires live Nequi credentials and is covered by integration/e2e tests.
"""
...
from app.integrations.nequi.mock_service import NequiServiceMock


# ── Helpers ───────────────────────────────────────────────────────────────────


def _make_mock() -> NequiServiceMock:
    """Return a fresh NequiServiceMock instance."""
    return NequiServiceMock()
...
class TestGenerateQrPayment:
    async def test_generate_qr_payment_returns_valid_response(self):
        """generate_qr_payment should return a NequiQRResponse with expected fields."""
        mock = _make_mock()

        result = await mock.generate_qr_payment(
            amount_cents=150_000,
            reference="abc123:some-ref",
            description="DentalOS Invoice INV-001",
        )

        assert result.payment_id.startswith("mock_")
        assert result.status == "pending"
        assert "mock.nequi.local" in result.qr_code_url
    ...
    async def test_generate_qr_payment_payment_id_is_deterministic(self):
        ...
        mock = _make_mock()
        reference = "tenant01:invoice-uuid"
        ...
        result_b = await mock.generate_qr_payment(
            amount_cents=200_000,  # different amount — must not affect ID
            reference=reference,
    ...
    async def test_generate_qr_payment_different_references_produce_different_ids(self):
        ...
        result_a = await mock.generate_qr_payment(
            amount_cents=100_000,
            reference="tenant01:invoice-aaa",
            description="Pago A",
        )
        result_b = await mock.generate_qr_payment(
            amount_cents=100_000,
            reference="tenant01:invoice-bbb",
            description="Pago B",
        )
...
class TestVerifyWebhookSignature:
    def test_verify_webhook_signature_valid_returns_true(self):
        ...
        # Any well-formed HMAC-SHA256 hex string
    ...
    def test_verify_webhook_signature_invalid_still_returns_true_in_mock(self):
        ...
        invalid_signature = "not-a-real-signature"
