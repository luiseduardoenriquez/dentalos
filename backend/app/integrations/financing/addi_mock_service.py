"""Mock Addi financing service for local development and testing.

Returns deterministic fake data so that the financing flow can be exercised
end-to-end without real Addi credentials. Automatically selected when
Addi env vars are empty.
...
logger = logging.getLogger("dentalos.integrations.financing.addi.mock")

# Addi financing thresholds (in COP cents)
_MIN_AMOUNT_CENTS = 100_000  # COP 1,000
_MAX_AMOUNT_CENTS = 50_000_000  # COP 500,000

# Standard installment options offered by Addi in Colombia
_AVAILABLE_INSTALLMENTS = [3, 6, 12, 24]


class AddiMockService(FinancingProviderBase):
    """Fake Addi service for development environments."""
        ...
        *,
        ...
        if in_range:
            ...
            result = EligibilityResult(
                ...
                reason="Monto fuera del rango permitido por Addi.",
            )

        logger.info(
            "Mock Addi eligibility: eligible=%s document=%s...",
            result.eligible,
    ...
    async def create_application(
        ...
        *,
        ...
        """Return a fake application creation response.

        The provider_reference is a UUID4 string prefixed with "addi_mock_"
        ...
        """
        provider_reference = f"addi_mock_{uuid.uuid4().hex[:16]}"

        result = ApplicationResult(
            provider_reference=provider_reference,
            status="pending",
            redirect_url=f"https://mock.addi.com.co/aplicacion/{provider_reference}",
        )

        logger.info(
            "Mock Addi application created: ref=%s...",
    ...
    async def get_status(
        ...
        *,
        ...
        References starting with "addi_mock_" are reported as "approved".
        ...
        is_approved = provider_reference.startswith("addi_mock_")
        status = "approved" if is_approved else "pending"

        result = ApplicationStatusResult(
            provider_reference=provider_reference,
            status=status,
            approved_amount_cents=5_000_000 if is_approved else None,  # COP 50,000 mock amount
            disbursed_at=datetime.now(UTC) if is_approved else None,
        )

        logger.info(
            "Mock Addi status queried: ref=%s... status=%s",
            provider_reference[:12],
            result.status,
        )
...
addi_mock_service = AddiMockService()
